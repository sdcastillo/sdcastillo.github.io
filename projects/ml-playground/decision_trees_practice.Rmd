...
<iframe src="https://player.vimeo.com/video/643153540?h=fa0c8776b0" width="640" height="360" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe> 
...
have very similar effects, while, when traffic is not controlled, intersections have higher Crash Score.  This is the same answer on the official SOA solution to June 13, 2019 Exam PA, Task 4.  See Task 4 of the Traffic Safety Solution video for a step-by-step example of finding interactions from graphs."),
...
<iframe src="https://player.vimeo.com/video/643153646?h=a77be49563" width="640" height="360" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen=""></iframe>
...
tree1 <- rpart(as.factor(value_flag) ~ .,
  ...
  method = "class",
  control = rpart.control(minbucket = 2, cp = 0.0002, maxdepth = 4),
...
quiz(
  question("Which set of parameters is best?",
    ...
    answer("**Parameters**: minbucket: [3,4], cp: [0.0001,0.01], maxdepth: [5,6]
           **Variables in splits**: marital_status, cap_gain, education_num, occupation, hours_per_week, age, score"  , correct = TRUE, message = "Choose the set of parameters from these answers which have the highest AUC based on the test set.  minbucket = 4, cp = 0.001, maxdepth = 6. All variables are used as splits in the tree."),
...
* `parent_edu`: Total level of education completed by either parent.  For each parent, the education level ranges from 0 to 4:. 

| Value of `parent_edu`   |      Definition      | 
|:----------:|:------------:|
| 0 | none | 
| 1 | primary education (4th grade)   |
| 2 | middle school (8th grade) |
| 3 | secondary education (high school) |
| 4 | higher education (college) |

For example, if the mother has secondary education (3) and the father has higher education (4) then `parent_edu` = 3 + 4 = 7
...
- **Joe**: Father has a PhD and mother has Bachelor's, Mother works in health, goes out infrequently (score of 3), health_alcohol score of 5
- **Steve**: Mother and Father have a middle school education, Mother works in civil services, goes out infrequently (score of 2), health_alcohol score of 5
- **Kim**: Father has an Associate's degree and mother has a Bachelor's degree, Mother works as an administrative assistant, goes out casually (score of 4), health_alcohol score of 4
...
```{r quiz-tree-2}
quiz(
  question("Choose the correct order, from the lowest predicted probability to the highest.",
    answer("Joe < Steve < Kim"),
    answer("Steve < Joe < Kim"),
    answer("Kim < Joe < Steve"),
    answer("Steve < Kim < Joe", correct = T, message = "We start from the top branch and working downwards.  A yes moves left and a no moves right. The predicted probability of passing is Y. 
...
## Q6 - Consider a decision tree

**SOA Exam PA June 13, 2019, Task 10**

You have just built a GLM which has 5 continuous predictor variables.  
...
i)	The tree automatically detects interactions whereas these need to be manually added to the GLM.
...
v)	The tree is biased towards selecting high cardinality and continuous features rather than categorical features with fewer values because more split points are possible.

vi)	In general, GLMs tend to have higher variance than decision trees, and this often leads to results that change significantly after training on new data.  
...
answer("ii, iii"),
answer("i, ii, vi, vi"),
answer("ii, v", correct = T, message = "ii) GLMs handle missing values by omitting them.  vi) A model has high variance if it is very sensitive to small changes in the training data.  This is another way of saying that it is 'flexible'.  Decision trees tend to be more flexible than GLMs because they can change their predictions drastically with small changes to the input data whereas GLMs can only change the predictions as the inputs change continuously.  Another way of saying this is that GLMs tend to have higher bias than decision trees."),
    allow_retry = TRUE
))
```


## Q7 - Calculate Gini index and entropy
**SRM Practice Question 9**: A classification tree is being constructed to predict if an insurance policy will lapse. A random sample of 100 policies contains 30 that lapsed. You are considering two splits:
...
```{r Q7, exercise=TRUE}
# calculate Gini index and entropy here
```


```{r Q7-solution}
# Gini index for split 1
2 * (20 * (12 / 20) * (8 / 20) + 80 * (18 / 80) * (62 / 80)) / 100

# Gini index for split 2
2 * (10 * (8 / 10) * (2 / 10) + 90 * (22 / 90) * (68 / 90)) / 100

# entropy for Split 1
-(20 * (12 / 20) * log(12 / 20) + 20 * (8 / 20) * log(8 / 20) + 80 * (18 / 80) * log(18 / 80) + 80 * (62 / 80) * log(62 / 80)) / 100

# entropy for Split 2
-(10 * (8 / 10) * log(8 / 10) + 10 * (2 / 10) * log(2 / 10) + 90 * (22 / 90) * log(22 / 90) + 90 * (68 / 90) * log(68 / 90)) / 100
```
...
quiz(
  question("Select the correct answer.",
    ...
    answer("(E) The correct answer is not given by (A), (B), (C), or (D).", 
           correct = TRUE,
           message = 'The total Gini index for Split 1 is 2[20(12/20)(8/20) + 80(18/80)(62/80)]/100 = 0.375 and for Split 2 is 2[10(8/10)(2/10) + 90(22/90)(68/90)]/100 = 0.3644. Smaller is better, so Split 2 is preferred. The factor of 2 is due to summing two identical terms (which occurs when there are only two classes).

The total entropy for Split 1 is –[20(12/20)ln(12/20) +20(8/20)ln(8/20) + 80(18/80)ln(18/80) + 80(62/80)ln(62/80)]/100 = 0.5611 and for Split 2 is –[10(8/10)ln(8/10) +10(2/10)ln(2/10) + 90(22/90)ln(22/90) + 90(68/90)ln(68/90)]/100 =
0.5506. Smaller is better, so Split 2 is preferred.
...
## Q8 - Advantages of decision trees
**SRM Practice Question 29**: Determine which of the following considerations may make decision trees preferable to other statistical learning methods.
